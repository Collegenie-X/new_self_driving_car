...
def rotate_servo_and_check_direction(car, detect_value, r_weight, g_weight, b_weight, y_value, direction_threshold):
    ...
    histogram_135 = np.sum(processed_frame, axis=0)
    center = int(np.sum(histogram_135[:len(histogram_135) // 2]))
    right_135 = int(np.sum(histogram_135[len(histogram_135) // 2:]))
    ...
    if not ret:
        ...
        return "STOP"
    processed_frame = process_frame(frame, detect_value, r_weight, g_weight, b_weight, y_value)
    histogram_45 = np.sum(processed_frame, axis=0)
    left_45 = int(np.sum(histogram_45[:len(histogram_45) // 2]))
    right_45 = int(np.sum(histogram_45[len(histogram_45) // 2:]))

    car.Ctrl_Servo(2, 90)
    time.sleep(1)

    if (right_135 - left_135) > (right_45 - left_45):
        return "LEFT"
    else:
        return "RIGHT"
...
try:
    while True:
        ...
        if direction == "UP":
            ...
            time.sleep(0.5)
            direction = rotate_servo_and_check_direction(car, detect_value, r_weight, g_weight, b_weight, y_value, direction_threshold)
